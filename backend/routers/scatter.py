# ==========================================================================
# PER 90 - SCATTER.PY (API ROUTER TIL SCATTER DATA MED DYNAMISK SUFFIX)
# ==========================================================================
...
def get_scatter_plot_data(
    ...
    # 1. 🎯 SUFFIX LOGIK PERFEKT INTEGRERET 🎯
    suffix = "_p90" if stat_type == "Per 90" else "_Total"
    
    # Lav en lokal kopi af datasættet
    ...
    try:
        ...
        # Find ud af hvilken positionskolonne der lever i dit CSV-ark
        pos_col = 'Pos.' if 'Pos.' in df.columns else ('Position' if 'Position' in df.columns else 'Position')

        # 3. LOOP IGENNEM ALLE SPILLERE OG BYG DATA-FEEDET
        players_list = []
        for _, row in df.iterrows():
            if pd.isna(row.get('Player Name')):
                continue

            # Vi bygger metrics for denne spiller ud fra de dynamiske titler
            player_metrics = {}
            ...
            # 🎯 Hent nationalitet og sikr mod NaN-støj
            nationality = str(row.get('Nationality', 'N/A')) if not pd.isna(row.get('Nationality')) else 'N/A'

            # Tilføj spilleren med sine stamdata (Inkl. Nationalitet!) og metrics-bundle
            players_list.append({
                ...
                "nationality": nationality,  # 🎯 INTEGRERET PERFEKT HER 1:1
                "age": int(row.get('Age', 0)) if not pd.isna(row.get('Age')) else 0,
                "mins_played": mins_played,
                "stats": player_metrics
            })

        return {
            "stat_type": stat_type,
            "suffix_used": suffix,
            "available_axes": list(custom_titles.values()),
            "players": players_list
        }

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Fejl under metrics generering: {str(e)}")
